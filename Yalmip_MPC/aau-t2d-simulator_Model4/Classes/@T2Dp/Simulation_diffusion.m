%Simulation function with diffusion:
function [X,time] = Simulation_diffusion(obj)
...
Tstop=[T(1) T(DgA>0) T(Ula>0) T(Ufa>0) T(Um>0) T(Uv>0) T(UGLP1h2>0) ...
    T(UGLP1h24>0) StressT HRT  Diffstops T(end)];
Tstop=unique(Tstop);
...
for i=2:length(Tstop)
   ...
   %Impulse inputs:
   Xstop([1 58 61 64 67 47 48 4 5 10 18 20 70 72],i-1)=[Dg(Tstop(i-1)+1);DgH(Tstop(i-1)+1);DgM(Tstop(i-1)+1);DgL(Tstop(i-1)+1);DgvL(Tstop(i-1)+1);DgA(Tstop(i-1)+1); DM; Um(Tstop(i-1)+1);Um(Tstop(i-1)+1);...
   ...
   if(ismember(Tstop(i-1),StressT))
       Stress = @(t)  StressType{Tstop(i-1)==StressT}(t,Tstop(i-1));
   end
   if(ismember(Tstop(i-1),HRT))
       HR = @(t)  HRType{Tstop(i-1)==HRT}(t,Tstop(i-1))+obj.Param.HRb;
   end
%    %Simulate:
   options  = odeset('RelTol',1e-3,'AbsTol',1e-3,'Jacobian', @(t,x) jacobian_fode_pot(obj,t,x, obj.LastMeal, Stress, HR, T,...
      obj.Param, obj.basal),'JConstant','no','BDF','on','MaxOrder',4);
   %options  = odeset('RelTol',1e-4,'AbsTol',1e-4);
   %tic
   [ts,Xs] =ode15s(@(t,x) fode(obj,t, x, obj.LastMeal, Stress, HR, T,...
       obj.Param, obj.basal), [Tstop(i-1) Tstop(i)],Xstop(:,i-1),options);
   Xs=Xs';
   X=[X,Xs(:,2:end)];
   Xstop(:,i)=Xs(:,end);
   Xstop(35,i) = Xstop(35,i) + obj.Param.sig_diff*sqrt(abs(Tstop(i)-Tstop(i-1)))*randn;
   %ttoc=toc;
   time=[time,ts(2:end)'];
   options.InitialStep = ts(end)-ts(end-1);
%    options.MaxStep = ts(end)-ts(1); 
