function DoseWithMeal = MPCWithMeal(obj, t, states)
%MPCWithMeal  Returns a dose based on the linear model prediction with meal
%
%MPCWithMeal(t, states, dt) where t is the current time, states is the
%previous states and dt is the timestep
    ...
    for k = obj.stepsPerDay:obj.Hp + (obj.stepsPerDay - 1)
        ...
        if(mod(k,obj.stepsPerDay)==0)
            ...
            if (k - obj.stepsPerDay < obj.Hu)
             ...
             xd = obj.A*(x - obj.xss')+ obj.B(:,1)*(insulinIdx - obj.uss)+obj.B(:,2)*(d(idx)- obj.dss);
            x = x + obj.timeStep*xd;
            ...
            xd = obj.A*(x - obj.xss') +obj.B(:,1)*(0 - obj.uss) +  obj.B(:,2)*(d(idx) - obj.dss);
            x = x + obj.timeStep*xd;
    ...
    tempu = value(u);
    DoseWithMeal = tempu(find(tempu, 1, 'first'));
