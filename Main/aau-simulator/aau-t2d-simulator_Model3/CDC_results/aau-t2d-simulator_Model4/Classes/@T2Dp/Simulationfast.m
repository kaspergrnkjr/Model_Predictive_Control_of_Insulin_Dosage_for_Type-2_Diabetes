%Simulation function:
function [X,time] = Simulationfast(obj)
%% Simulation time: 
Nd=obj.Nd; %days
Nmin=obj.Nmin;
T=obj.T;
...
for i=2:length(Tstop)
   ...
   if(something==1 && i==2 && obj.CLmode == 0)
       DM=1;
   elseif (i==2 && obj.CLmode == 1 && Dg(Tstop(i-1)+1)==0)
       DM=0;
   elseif not(Dg(Tstop(i-1)+1)==0)
   ...
   %Simulate:
   tic
   %ts = linspace(Tstop(i-1),Tstop(i),5000);
   %options = odeset('AbsTol',1e-9,'RelTol',1e-9);
   [ts,Xs] =ode15s(@(t,x) fode(obj,t, x, obj.LastMeal, stressv, HRv, T,...
